...
exports.copyBuildOutput = copyBuildOutput;
...
function copyBuildOutput(id) {
    // Try dist folder first
    let folderPath = path_1.default.join(__dirname, `output/${id}/dist`);
    let outputPrefix = 'dist';
    // If dist doesn't exist, try build folder
    if (!fs_1.default.existsSync(folderPath)) {
        folderPath = path_1.default.join(__dirname, `output/${id}/build`);
        outputPrefix = 'build';
        // If neither exists, log error and return
        if (!fs_1.default.existsSync(folderPath)) {
            console.error(`Neither dist nor build directory exists in output/${id}/`);
            return;
        }
    }
